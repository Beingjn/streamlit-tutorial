...
with tab_native:
    ...
    # Line
    _line_df = df.dropna(subset=["date", "sale_price"]).set_index("date")[["sale_price"]].sort_index().loc['2023-01-01':'2023-1-8']
    ...
    # Scatter
    _scatter_df = df_top_cities[['size', 'sale_price', 'city']].dropna(subset=['size', 'sale_price'])
    st.scatter_chart(_scatter_df, x='size', y='sale_price', color='city', use_container_width=True)
    st.code(
        "st.scatter_chart(df_top_cities[['size','sale_price','city']].dropna(subset=['size','sale_price']), x='size', y='sale_price', color='city')",
        language="python",
    )
